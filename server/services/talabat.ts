  ...
  async searchProducts(query: string): Promise<Product[]> {
    try {
      ...
      try {
        ...
        console.log('Talabat API endpoint failed, falling back to HTML scraping', apiError);
      }
  ...
  async getProductDetails(productId: string): Promise<Product | null> {
    try {
      ...
      try {
        ...
        if (apiResponse.status === 200 && apiResponse.data) {
          console.log('Successfully used Talabat API endpoint for product details');
          return this.parseProductDetails(apiResponse.data, productCode);
        }
      } catch (apiError) {
        console.log('Talabat API endpoint failed for product details, falling back to HTML scraping', apiError);
      }
        ...
        for (const item of data.items) {
          ...
          try {
            ...
            if (item.originalPrice) {
              originalPrice = typeof item.originalPrice === 'number' ? 
                item.originalPrice : 
                parseFloat(String(item.originalPrice).replace(/[^\d.]/g, ''));
            }
            ...
            const rating = item.rating ? parseFloat(String(item.rating)) : undefined;
            const reviewCount = item.reviewCount ? parseInt(String(item.reviewCount), 10) : undefined;
            ...
            products.push({
              ...
              originalPrice,
              discount,
              image: image || `https://via.placeholder.com/300x300?text=${encodeURIComponent(title)}`,
              url,
              platform: 'talabat',
              rating,
              reviewCount,
  ...
  private parseProductDetails(data: any, productCode: string): Product | null {
    try {
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        let price = 0;
        if (data.price) {
          price = typeof data.price === 'number' ? data.price : parseFloat(String(data.price).replace(/[^\d.]/g, ''));
        }
        
        // Original price if available
        let originalPrice: number | undefined;
        if (data.originalPrice) {
          originalPrice = typeof data.originalPrice === 'number' ? 
            data.originalPrice : 
        ...
        if (originalPrice && price && originalPrice > price) {
          ...
          discount = typeof data.discount === 'number' ? 
            data.discount : 
        ...
        // Rating and review count if available
        const rating = data.rating ? parseFloat(String(data.rating)) : undefined;
        const reviewCount = data.reviewCount ? parseInt(String(data.reviewCount), 10) : undefined;
        ...
        return {
          ...
          description,
          isFreeDelivery,
          isPromotional: isPromotional || (discount !== undefined && discount > 0),
          brand,
          specs,
          isBestPrice: false
        };
